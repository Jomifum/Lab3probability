...
#Streak of 1: One hit followed by a miss (H M).
#Streak of 0: A miss with no preceding hits (M).
# In streak of 1 there is one hit followed vt a miss 1 in the same streak meaning there are one hit and one missed hit as well ithe first right.
...
str(kobe_streak)  # Check the structure of the output
head(kobe_streak)  # View the first few entries

colnames(kobe_streak) <- "length" # rename the column kobe_streak to length
```
...
ggplot(data = kobe_streak, aes(x = length)) +
  geom_bar() +
  labs(title = "Distribution of Kobe's Streak Lengths",
       x = "Streak Length (Number of Consecutive Hits)",
       y = "Frequency")
```
...
**Insert your answer here**
# the column kobe_streak has to be rename to length in order to get the plot using the chunck
#code given bebore using colnames(kobe_streak) <- "length"
#According the plot regarding  a 38 of Frequency (count) that an streak was not hit, then 1 streak was hit with a frequency of 29, two streaks were hit with a frequency of 6 and same for
# 3 streaks, and last 4 streaks were hit with a frequency of 1, seeing a descending tendency of frequency of Kobe's streak hitten. 
...
**Insert your answer here**
#Sample was provided in the chuck code below
...
**A note on setting a seed:** Setting a seed will cause R to select the same sample each time you knit your document. This will make sure your results don't change each time you knit, and it will also ensure reproducibility of your work (by setting the same seed it will be possible to reproduce your results). You can set a seed like this:
  ...
  ```{r set-seed}
set.seed(12345)                 # make sure to change the seed

# Define the outcomes of the coin
coin_outcomes <- c("heads", "tails")

# Simulate flipping the unfair coin 100 times with specified probabilities
sim_unfair_coin <- sample(coin_outcomes, size = 100, replace = TRUE, prob = c(0.2, 0.8))

# View the results of the simulation
sim_unfair_coin

# Count how many times heads and tails occurred
...
```{r sim-basketn}
# Set the seed below for reproducibility
set.seed(98765)

# To define the outcomes of the shot:
shot_outcomes <- c("H", "M")

# This simulates 133 shots with a 45% chance of making a basket ("H") and a 55% chance of missing ("M")
sim_basket <- sample(shot_outcomes, size = 133, replace = TRUE, prob = c(0.45, 0.55))

# View the results of the simulation
sim_basket

# Count how many hits ("H") and misses ("M") occurred in the simulation
table(sim_basket)
...
**Insert your answer here**
```{r sim-basket-ot}
# Compute the streak lengths of the simulated basket results
sim_streak <- calc_streak(sim_basket)

# View the first few entries of the resulting data frame
head(sim_streak)
...
**Insert your answer here**
  ```{r sim-basketsn}
# Assuming `calc_streak` has been defined and executed on `sim_basket`
sim_streak <- calc_streak(sim_basket)

# Check the structure of sim_streak to verify column names
str(sim_streak)

# Rename the column to 'length' if it's not already named that (or adjust as necessary)
# Assuming the column with streak lengths is called 'kobe_streak' based on previous output
colnames(sim_streak) <- c("length")  # Rename the column to 'length' for consistency

# Convert length to factor
sim_streak$length <- as.factor(sim_streak$length)

# Calculate summary statistics
streak_summary <- table(sim_streak$length)  # Summary of counts for each streak length
typical_streak_length <- mean(as.numeric(levels(sim_streak$length)[as.integer(sim_streak$length)]), na.rm = TRUE)  # Calculate the mean streak length
longest_streak_length <- max(as.numeric(levels(sim_streak$length)[as.integer(sim_streak$length)]), na.rm = TRUE)  # Find the max streak length

# Plot the streak lengths
ggplot(data = sim_streak, aes(x = length)) +
  geom_bar() +
  labs(title = "Distribution of Streak Lengths for Simulated Shooter",
       x = "Streak Length",
       y = "Count") +
  theme_minimal()
...
print(typical_streak_length)
print(longest_streak_length)

str(sim_streak)
```
#Analysis: The simulated independent shooter with a 45% shooting percentage demonstrates a tendency to miss frequently, as evidenced by the high occurrence of streaks with length of 0. The average streak length of around 0.72 indicates that the shooter can have moments of success, they regularly do not sustain long streaks of baskets, with the longest streak being just 3 continuos baskets. This aligns well with the expectations for a shooter with a less-than-50% shooting percentage.
...
**Insert your answer here**
  #If I were to run the independent shooter simulation again, I would expect the streak distribution to be somewhat similar to the previous run, but not identical. This is due to several factors: the randomness and independence of each shot, which causes variations in streak lengths; the general pattern where shorter streaks are more common; the Central Limit Theorem, which indicates that multiple simulations will better represent the shooting percentage; and the variation in specific outcomes, such as the longest streak or the number of zero-length streaks. Overall, while similar trends reflecting the 45% shooting percentage are expected, the exact counts and specific outcomes will likely differ.
...
**Insert your answer here**
  #Kobe Bryant's streak lengths differ noticeably from the simulated shooter, which reflects a 45% shooting percentage and more frequent short streaks. If Kobe's data shows longer and more frequent streaks, it supports the "hot hand" theory, suggesting he sometimes shot exceptionally well. Conversely, if his patterns match the simulation, his performance might not differ significantly from random chance. Analyzing these streaks can help determine if Kobe's shooting aligns with the hot hand theory, highlighting his ability to score consistently in bursts.
